class ImageConstant {
  // Image folder path
  static String imagePath = 'assets/images';
  static String iconPath = 'assets/icons';

  // sign up images
  static String imgFfddf1 = '$imagePath/img_ffddf_1.png';

  // templates images
  static String imgTrash2 = '$imagePath/img_trash_2.svg';

  // Profile Edit images
  static String imgSend = '$imagePath/img_send.svg';

  static String imgUnsplashJmurdhtm7ng =
      '$imagePath/img_unsplash_jmurdhtm7ng.png';

  static String imgArrowLeft = '$imagePath/img_arrow_left.svg';

  // reset password images
  static String imgDfVue13 = '$imagePath/img_df_vue_13.png';

  // check emails images
  static String imgEmail1 = '$imagePath/img_email_1.png';

  // new password images
  static String imgDfVue14 = '$imagePath/img_df_vue_14.png';

  // start  images
  static String imgDfVue61 = '$imagePath/img_df_vue6_1.png';

  static String imgNnnj1 = '$imagePath/img_nnnj_1.png';

  static String imgPp4 = '$imagePath/img_pp_4.png';

  // voice recognition images
  static String imgFrrfr1 = '$imagePath/img_frrfr_1.png';

  static String imgVoiceSearch32 = '$imagePath/img_voice_search3_2.png';

  // edit settings images
  static String imgNoisePollution = '$imagePath/img_noise_pollution.png';

  static String imgOnButton1 = '$imagePath/img_on_button_1.png';

  static String imgMic = '$imagePath/img_mic.svg';

  static String imgMicWhiteA700 = '$imagePath/img_mic_white_a700.svg';

  // edit fonts images
  static String imgMinus = '$imagePath/img_minus.svg';

  // iPhone 13 & 14 - Two images
  static String imgCdddf2520x390 = '$imagePath/img_cdddf_2_520x390.png';

  static String img1000F20937617 = '$imagePath/img_1000_f_20937617.png';

  // Otp Verifcation images
  static String imgEllipse23 = '$imagePath/img_ellipse_23.png';

  static String imgEllipse21 = '$imagePath/img_ellipse_21.png';

  static String imgGroup = '$imagePath/img_group.svg';

  // Submit otp images
  static String imgEllipse23294x390 = '$imagePath/img_ellipse_23_294x390.png';

  static String imgEllipse21218x168 = '$imagePath/img_ellipse_21_218x168.png';

  static String imgRating = '$imagePath/img_rating.svg';

  // Common images
  static String imgSignup3 = '$imagePath/img_signup_3.png';

  static String imgHhuuhuh2 = '$imagePath/img_hhuuhuh_2.png';

  static String imgHide1 = '$imagePath/img_hide_1.png';

  static String imgCdddf2 = '$imagePath/img_cdddf_2.png';

  static String imgFacebook1 = '$imagePath/img_facebook_1.png';

  static String imgGoogle1 = '$imagePath/img_google_1.png';

  static String imgApple1 = '$imagePath/img_apple_1.png';

  static String imgClose = '$imagePath/img_close.svg';

  static String imgCdddf3 = '$imagePath/img_cdddf_3.png';

  static String imgFavorite = '$imagePath/img_favorite.svg';

  static String imgArrowDown = '$imagePath/img_arrow_down.svg';

  static String imgTrash2Errorcontainer =
      '$imagePath/img_trash_2_errorcontainer.svg';

  static String imgSliders = '$imagePath/img_sliders.svg';

  static String imgDownload = '$imagePath/img_download.svg';

  static String imgVoiceSearch4 = '$imagePath/img_voice_search_4.png';

  static String imgXCircle = '$imagePath/img_x_circle.svg';

  static String imgNavSettings = '$imagePath/img_nav_settings.svg';

  static String imgNavColors = '$imagePath/img_nav_colors.svg';

  static String imgNavFonts = '$imagePath/img_nav_fonts.svg';

  static String imageNotFound = 'assets/images/image_not_found.png';
  static String frrfr = '$imagePath/frrfr.png';
  static String apple = '$imagePath/apple.png';
  static String audioWaves = '$imagePath/audio-waves.png';
  static String dfVuelogo = '$imagePath/df_vue_logo1.png';
  static String dfVuelogoWithName = '$imagePath/df_vue_with_name.png';
  static String email = '$imagePath/email.png';
  static String facebook = '$imagePath/facebook.png';
  static String ffddf = '$imagePath/ffddf.png';
  static String google = '$imagePath/google.png';
  static String hhuuhuh = '$imagePath/hhuuhuh.png';
  static String hide = '$imagePath/hide.png';
  static String lock = '$imagePath/lock.png';
  static String message = '$imagePath/message.png';
  static String mmdccddf = '$imagePath/mmdccddf.png';
  static String nnnj = '$imagePath/nnnj.png';
  static String noisePollution = '$imagePath/noise-pollution.png';
  static String onButton = '$imagePath/on-button.png';
  static String padlock = '$imagePath/padlock.png';
  static String phoneReceiverSilhouette =
      '$imagePath/phone-receiver-silhouette.png';
  static String signUp = '$imagePath/sign-up.png';
  static String voiceSearch = '$imagePath/voice-search.png';
  static String voiceSearch3 = '$imagePath/voice-search3.png';
}
